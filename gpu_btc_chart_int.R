...
btc <- read.csv(paste0("https://raw.githubusercontent.com/cjrieth/AC-5GroupProject/main/data/", switch(output$gpu, "Bitcoin" = "BTC-USD-5Y.csv", "Ethereum" = "ETH-USD-MAX.csv", "Lite Coin" = "LTC-USD-MAX.csv")), na.strings = c("null"))
...
gpu_btc_plot <- ggplot(joined, aes(x = Date, y = Close)) +
  ...
  ylim(0, 1000) +
  labs(color = "New GPU Released", y = "Close Price (USD)", title = paste0("Overlay of new GPU Releases with ", output$gpu, " price"))
